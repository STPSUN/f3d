...
//        $this->assign('loadPrice', 'getPrice');
        ...
        $current_days = $m->getValByName('total_days'); //P3D历时天数

        $data['daily_avg_amount'] = $total_token_amount / $current_days; //P3D历时天数
        $data['daily_avg_bonus'] = $total_token_bonus / $current_days; //每日收益

        $tokenRecordM = new \addons\fomo\model\TokenRecord();
        $data['total_token_num'] = $tokenRecordM->getTotalToken(); //购买总量
