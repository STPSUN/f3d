...
class EthTradingOrder extends \web\common\model\BaseModel {
    ...
    public function getUncheckDataByID($id){
        ...
        $sql = 'select a.*,b.contract_address,b.byte from '.$this->getTableName().' a ,'.$m->getTableName().' b where a.id= '.$id.' and a.status=0 and a.txhash="" and a.coin_id=b.id limit 0,1';
        $data = $this->query($sql);
        if(!empty($data)){
            return $data[0];
        }else{
